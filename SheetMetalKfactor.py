...
class KFactorLookupTable:
    ...
    def __init__(self, material_sheet):
        ...
        if len(lookup_sheet) >= 1:
            ...
            raise ValueError("No spreadsheet found containing material definition: %s" % material_sheet)
        ...
        k_factor_lookup = self.build_k_factor_lookup(lookup_sheet, key_column_name, key_column_row, value_column_name)

        options_cell = self.find_cell_by_label(lookup_sheet, 'Options')
        k_factor_standard = self.get_k_factor_standard(lookup_sheet, options_cell, k_factor_standard)
