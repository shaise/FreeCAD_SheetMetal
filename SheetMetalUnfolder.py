...
def getUnfoldSketches(
    ...
    try:
        unfoldobj_label = obj.Label + "_Unfolded"
    except:
        unfoldobj_label = "Unfolded"
    ...
    unfold_sketch = generateSketch(edges, f"{unfoldobj_label}_Sketch", sketchColor)
    ...
    unfold_sketch_outline = generateSketch(owEdgs, f"{unfoldobj_label}_Sketch_Outline", sketchColor
                                           existingSketches)
    sketches.append(unfold_sketch_outline)
    if tidy:
        SMLogger.error(FreeCAD.Qt.translate("Logger", f"tidying up {unfoldobj_label}_Sketch_Outline"))
    intEdgs = []
    idx = []
    ...
    for i, e in enumerate(faceEdgs):
        if i not in idx:
            intEdgs.append(e)
    if len(intEdgs) > 0:
        unfold_sketch_internal = generateSketch(intEdgs, f"{unfoldobj_label}_Sketch_Internal", 
                                                internalSketchColor, existingSketches)
        sketches.append(unfold_sketch_internal)
    if len(foldLines) > 0 and splitSketches:
        unfold_sketch_bend = generateSketch(foldEdges, f"{unfoldobj_label}_Sketch_bends", bendSketchColor
                                            existingSketches)
